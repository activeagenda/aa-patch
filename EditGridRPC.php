...
if( !empty($ModuleID) ){
    if( in_array($subModuleID, array( 'ntf','att','cos','lnk','nts', 'modnr', 'rmd' )) ){
        $isGlobal = true;
...
trace($formResult, '$formResult');
            ...
            if( count($cells) > 0 ){
                ...
                if( $isConsistent ){
                    $quickedit_img = "grqckedtd";
                    $quickedit_msg = gettext("Quick Edit");
                }else{
                    $quickedit_img = "grincstncy";
                ...
                $firstCell = array(
                    'className'=>'l',
                    'id'=>$returnRowID,
                    'innerHTML'=>"<a class=\"l $quickedit_img\" titel=\"$quickedit_msg\" href=\"javascript:editRow('$returnRowID','$subModuleID')\"></a>"
                );
				if( true == $grid->quickEditAdd OR true == $grid->quickEditOnly ){
					$firstCell = array(
						'className'=>'l',
						'id'=>$returnRowID,
						'innerHTML'=>"<a class=\"l $quickedit_img\" titel=\"$quickedit_msg\" href=\"javascript:editRow('$returnRowID', '$subModuleID')\"></a>"
        ...
        default:
            $content = '';
            $error = gettext("ERROR: Unknown action requested.");
