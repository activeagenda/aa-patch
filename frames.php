...
if( !empty($_GET['mdl']) and !empty($_GET['rid']) and empty($_GET['dest'])){
    $dest = 'view.php?mdl='.$_GET['mdl'].'&rid='.$_GET['rid'];
}

if( !empty($_GET['mdl']) and empty($_GET['rid']) and empty($_GET['dest'])){
    $dest = 'list.php?mdl='.$_GET['mdl'];
}
...
<frameset rows="52, *" framespacing="0" frameborder="0" border="0">
    ...
    <frame name="main" src="<?php echo $framedest; ?>" />
</frameset>
</html>