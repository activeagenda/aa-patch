...
function renderHTML($selModuleID = '')
{
    ...
    $content = '';
    $counter = 1;
	$navigation_list = $this->Items[0]->Items;
...
function renderHTML($showClassName = FALSE)
{
    if( $this->phrase == 'Navigate' ){
		...
		if( $this->nodeFolded ){
			$class = ' class="pad1NF"';
			$expandlink = '<a class="mndown" id="MT'.$liID.'" href="javascript:menu_toggel(\''.$liID.'\')"></a>';
		}else{
			$class = ' class="pad1"';
			$expandlink = '';
		}
        ...
        if( $showClassName ){
            $content = '<li '.$class.' id="'.$liID.'">'.get_class($this) .' '. gettext($this->phrase); 
        }else{
            $content = '<li '.$class.' id="'.$liID.'"><b>'.gettext($this->phrase).'</b>'.$expandlink; 
        }
        $content .= $this->renderChildrenHTML();
...
function NavCategory( $pPhrase, $nodeFolded )
{
...
function NavInternalLink($pPrimary, $pSecondary, $pFrame, $pExpand, $pNewBrowser, $pPhrase, $nodeFolded){
...
function renderHTML($showClassName = FALSE)
{
    ...
    $content = '<li '.$class.' id="'.$liID .'">';
    if( $showClassName ){
        $content .= get_class($this);
    } 
    $content .= '<a href="'.$linkto.'"><b>'.gettext($this->phrase).'</b></a>'.$expandlink ; 
    $content .= $this->renderChildrenHTML();
...
{
var $target;
function NavExternalLink($pTarget, $pPhrase, $nodeFolded)
{
    $this->target = $pTarget;
    $this->phrase = NavItem::checkPhrase(ShortPhrase($pPhrase));
    $this->longPhrase = NavItem::checkPhrase(LongPhrase($pPhrase));
...
function NavModuleLink($pModuleID, $pPhrase, $primary, $nodeFolded)
{
...
function renderHTML($showClassName = FALSE)
{
    ...
    if( $User->PermissionToView($this->moduleID) ){
        $liID = uniqid();
		if( $showClassName ){
            $content = '<li id="'.$liID.'">'.get_class($this) .' '. gettext($this->phrase); 
        }else{
            if( count($this->Items) == 0 ){
				if( $this->nodeFolded ){
					$class = ' class="pad1NF"';
            ...
            $content = '<li'.$class .' id="'.$liID.'"><b><a'.$class.' href="'.$this->primary.'">'.gettext($this->phrase).'</a></b>'.$expandlink; 
        }
        $content .= $this->renderChildrenHTML();
        $content .= "</li>";
        return $content;
