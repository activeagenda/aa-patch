...
function PermissionGrid($pModuleID, $pPhrase)
{
    ...
    $field1 = MakeObject( $pModuleID, 'ModuleID', 'ViewField', array('phrase'=>'ID') );
	$this->AddField($field1);
    $field2 = MakeObject( $pModuleID, 'Module', 'ViewField', array('phrase'=>'Module') );
    ...
    $this->listSQL = "SELECT `mod`.ModuleID, `mod`.Name AS Module, `mod`.OwnerField, `{$this->moduleID}`.EditPermission, `{$this->moduleID}`.ViewPermission FROM `mod` LEFT OUTER JOIN `{$this->moduleID}` ON `mod`.ModuleID = `{$this->moduleID}`.ModuleID AND `{$this->moduleID}`._Deleted = 0 AND 
	 (ISNULL(`{$this->moduleID}`.{$this->idFieldName}) OR `{$this->moduleID}`.{$this->idFieldName} = /*recordID*/) WHERE `mod`.ParentModuleID = '' ";
	$this->valueListSQL = '';
    $this->ParentRowSQL = '';
...
function render($page, $qsArgs)
{
    ...
    $content = sprintf(
        VIEWGRID_MAIN,
        '<div id="usrpPG">'.gettext($this->phrase).'</div>',
...
//print debug_r($insertValues, 'inserts');
        ...
        foreach( $updateValues as $type => $values ){
            foreach( $values as $value => $modIDs ){
                ...
                $updateSQL = "UPDATE `{$this->moduleID}` 
				 SET {$type}Permission = {$value}, _Deleted = 0, _ModDate = NOW(), _ModBy = {$User->PersonID} 
				 WHERE `{$this->idFieldName}` = $recordID AND ModuleID IN ($updateModuleIDs)";
//print debug_r($updateSQL)."<br />\n";
        ...
        if( count($insertValues) > 0 ){
            $insertSQL = "INSERT INTO `{$this->moduleID}` ({$this->idFieldName}, ModuleID, EditPermission, ViewPermission, _ModDate, _ModBy, _Deleted ) VALUES ";
            foreach( $insertValues as $modID => $values ){
                $editVal = intval($values['edit']);
                $viewVal = intval($values['view']);
                $insertSQLs[] = "($recordID, '$modID', {$editVal}, {$viewVal}, NOW(), {$User->PersonID}, 0)";
            }
