...
// START
// Inserting triggers into the database
if(isWindows()){
...
if ( filesize( $trigger_file ) > 0 ){
	print "s2a: Installing triggers...\n";
	if( empty( $dbrootPassword ) ){
		$dbrootPassword = textPrompt("Please enter the root password for the database");
	}
	shellCommand($mysql_command .' --user=root --password='.$dbrootPassword.' '.DB_NAME.' < "'.$trigger_file.'"', false);
}

$script_file = XML_PATH.'/script.sql';
if ( file_exists( $script_file ) AND filesize( $script_file ) > 0){
	print "s2a: Running custom SQL script...\n";
	if( empty( $dbrootPassword ) ){
		$dbrootPassword = textPrompt("Please enter the root password for the database");
	}
	shellCommand($mysql_command .' --user=root --password='.$dbrootPassword.' '.DB_NAME.' < "'.$script_file.'"', false);
}

$script_file = $script_location.'/s2a-pomo';
if ( file_exists( $script_file ) AND isset( $Language ) ){
	print "s2a: Running translation file merge and compilation...\n";	
	shellCommand('bash '.$script_file." $Language", false);
}
//END
...
if( count($failedModules) > 0 ){
    ...
    print "  s2a: Some modules were not generated correctly:  \n" ;
    print "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n" ;
    print "\n";
    print join("\n\n", $failedModules);
    print "\n\n";
