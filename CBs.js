...
function ReloadPeople(sFormName, sFieldName, selOrgID, arPeople){
	...
	if( document.forms[sFormName].elements['fm_'+sFieldName] ){
            try {
                document.forms[sFormName].elements['fm_'+sFieldName].value = '';
            }
            catch(e) {
...
function UpdateFields(sFormName, oParentField, arChildFieldNames){
    ...
    while(ctr < len){
        ...
        if( document.forms[sFormName].elements[sChildName] ){
			if( document.forms[sFormName].elements[sChildName].nodeName.toLowerCase() == 'select' ){
				try {
			...
			if( document.forms[sFormName].elements[sChildName].nodeName.toLowerCase() == 'input' ){
				try { 
					document.forms[sFormName].elements[sChildName].value = oParentField.value;
				}
				catch(e) {
				   //would be nice to only suppress ReferenceErrors here but browser implementations differ (IE returns TypeError)
				}
		...
		if( document.forms[sFormName].elements['fm_'+sChildName] ){
            try {
                document.forms[sFormName].elements['fm_'+sChildName].value = '';
            }
            catch(e) {
               //would be nice to only suppress ReferenceErrors here but browser implementations differ (IE returns TypeError)
            }
...
function PopulateOrganizationCB(sFormName, sOrgFieldName, sPersonFieldName, selOrganizationID, arPeople, arOrgs){

    //get references to the select fields
    var oOrgField = document.forms[sFormName].elements[sOrgFieldName];
    var oPersonField = document.forms[sFormName].elements[sPersonFieldName];
    
    var selOrgID = selOrganizationID;
