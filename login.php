    ...
    if (isset($_GET['dest'])) {
        ...
        if(false !== strpos($dest, 'frames.php')){
			...
			if( empty( $myURL[1] ) ) {
				...
				if( false !== strpos($myURL[1], 'mdl=') and false === strpos($myURL[1], 'rid=') ){
					$dest = str_replace( 'frames.php', 'list.php', $dest);
					$dest_enc = base64_encode($dest);
					$RedirectTo = "dest=" . str_replace('\\', '', $dest_enc);
				}elseif( false !== strpos($myURL[1], 'mdl=') and false !== strpos($myURL[1], 'rid=')){
					$dest = str_replace( 'frames.php', 'view.php', $dest);
					$dest_enc = base64_encode($dest);
					$RedirectTo = "dest=" . str_replace('\\', '', $dest_enc);
				}elseif( false !== strpos($myURL[1], 'dest=') ){
					$RedirectTo = $myURL[1];
				}else{
					$dest = 'home.php';
					$dest_enc = base64_encode($dest);
		...
		$dest_enc = base64_encode('home.php');
        $RedirectTo = "dest=" . str_replace('\\', '', $dest_enc);
    }
    ...
    if( isset( $_POST['UserName']) ){
        ...
        if( empty($_COOKIE[SITE_SESSIONNAME]) ){
            ...
            if( $User->Login( $UserName, $Password ) ){
                ...
                $SQL = "SELECT Link, Title FROM usrds WHERE _Deleted=0 AND PersonID = {$User->PersonID} ORDER BY Type, Title";
                ...
                $msg = gettext("Login failed !") . "<br><br>\n";
                $msg .= gettext("Please check the spelling of username and password.")." \n";
