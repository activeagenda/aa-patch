...
function verifyPassword( $fieldName, $emsg=true )
{
    global $messages;
    ...
    if( empty($pass) ){
		...
		if( empty($recordID) ){
			if( $emsg ){
				$messages[] = array('e', gettext("The passwords must be min. 8 characters long, at least: 1 lower case, 1 upper case, 1 digit"));
			}
		}else{
			if( $emsg ){
				$messages[] = array('e', gettext("No password change"));
			}
			global $data;
	...
	if( array_key_exists( $fieldName.'_confirm', $_POST) ){
		if( $_POST[$fieldName] !== $_POST[$fieldName.'_confirm'] ){
			if( $emsg ){
				$messages[] = array('e', gettext("The passwords you supplied did not match"));
			}else{
				$messages[] = array( 'e', nl2br( gettext("The record has not been saved, because:")."\n".'- '
				.lcfirst( gettext("The passwords you supplied did not match") ) ) );
			}
		return false;
		}
	}		
	...
	$policy = '/^.*(?=.{8,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).*$/';
	if( preg_match( $policy, $_POST[$fieldName], $matches ) ){
		...
		if( $emsg ){
			...
			$messages[] = array( 'e', nl2br( gettext("The record has not been saved, because:")."\n".'- '
			 .lcfirst( gettext("The passwords must be min. 8 characters long, at least: 1 lower case, 1 upper case, 1 digit") ) ) );
		}
		return false;
	}  
}
