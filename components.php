...
function render(&$values, &$phrases)
{
    ...
    if($this->isSubField()){
		...
		$LongPhrase = LongPhrase($phrase);
		$ShortPhrase = ShortPhrase($phrase);
//echo ':'.$LongPhrase.':';
//echo '-'.$LongPhrase.'-';		
		if( $LongPhrase == ' ' AND $ShortPhrase == ' ' ){
			$content = sprintf(
				FIELD_HTML_HRULER,
				LongPhrase($phrase),
				ShortPhrase($phrase),
...
function viewRender(&$values)
{
	...
	switch( $this->dataType ){
		...
		case 'datetime':
			...
			if( (null == $rawValue) || ('0000-00-00' == $rawValue) ) {
				...
				$content = str_replace( ' ', '<span class="dttmv">', $rawValue );
		...
		case 'money':
			...
			// floor on float gests somtiems eg. 4.6 wrong results  so string cast needed!
			if( !empty($rawValue) ){
				$nVal = 100 * $rawValue;
				if( strpos( (string) $nVal, "." ) == false 
					AND strpos( (string) $nVal, "," ) == false ){
					$content = number_format($rawValue, 2, DECIMAL_SEPARATOR, ' ').' &nbsp;'.MASTER_CURRENCY;
				} else { //there are more than 2 decimals so display the whole thing
					$content = number_format($rawValue, 4, DECIMAL_SEPARATOR, ' ').' &nbsp'.MASTER_CURRENCY;
				}
			}else{							
				$content = $rawValue;
			}
			break;
		case ( preg_match( '/varchar(\d*)/', $this->dataType ) ? true : false ):
		case 'text':
			//longer character fields
			//$content = nl2br($rawValue);			
			$rawValue = str_replace( '!mailtoRecordTopicSubject!', $mailtoRecordTopicSubject, $rawValue );
			$rawValue = str_replace( '!themeDirectory!', $theme_web, $rawValue );
			$content = $rawValue;
			break;
		default:
			if( is_numeric($rawValue) ){
				if( '' != $this->displayDecimals ){
					...
					$content = number_format($rawValue, $this->displayDecimals, DECIMAL_SEPARATOR, ' ');
				}else{
					//$content = str_replace( '.', DECIMAL_SEPARATOR, $rawValue );
					$numberOfDecimals =  strlen(substr(strrchr($rawValue, "."), 1));
					$content = number_format($rawValue,$numberOfDecimals, DECIMAL_SEPARATOR, ' ');
				}
			}else{
				$content = $rawValue;
			}
    ...
    if( !empty($this->displayFormat) ){
		...
		if( preg_match( '/^\s*\/.*\/.*\/[ismxeuADSUX]*\s*/' , $this->displayFormat ) ){
			preg_match( "/^\s*\/(.*)\/(.*)\/([ismxeuADSUX]*)\s*/", $this->displayFormat, $matches );
...
function getDefaultValue($data = null)
{
    ...
    case 'defaultorgID':
        ...
        break;
	case 'now':
        return strftime( $User->getDateTimeFormatPHP() ); //defaults to current "date time"
        break;	
    case 'today':
		global $User;
        return strftime( $User->getDateFormatPHP() ); //defaults to current date
        break;	
    default:
...
function getSearchPhrase(&$data, &$moduleFields)
{
    ...
    $content .= strip_tags( ShortPhrase($phrase) ). ': ' . $data[$this->name];
    return $content;
}
...
function ViewField(&$element, $moduleID)
{
	...
	$this->linkIcon = $element->getAttr('icon');
	...
	// navigation to the form on edit.php
	$this->goEditScreen = $element->getAttr('goEditScreen');
...
//                $content .= $this->viewRender($values);
				...
				if( !empty( $RelatedRecordText ) ){ 
					if( defined('IS_RPC') && IS_RPC  ){
						...
						$content .= '<div>';
						$content .= '<a class="rcdnwwnd" href="frames.php?dest='.$poplink.'" target="_blank" ';
                ...
                if( $newWin ){
					...
					$extlinkIcon = '<img src="'.$theme_web.'/img/'.$linkIcon.'"/> ';
                }else{
                    $str_target = '';
					$extlinkIcon = '';
                }
...
function simpleRender(&$values)
{
	...
	$edit_link = '';
	$callingModuleID = $qsArgs['mdl'];
	$EditScrPermission = $User->ModulesPerm->$callingModuleID;
	if( isset( $this->goEditScreen ) AND 
	 ( $User->PermissionToEdit( $callingModuleID )>0 OR !empty( $EditScrPermission[$this->goEditScreen] ) ) ){
		$callParams = $qsArgs;
		unset( $callParams['scr'] );
		$callParams = '&amp;'.MakeQS( $callParams );
		$edit_link = 
		'<div id="editCmd"><a class="gEScr" href="edit.php?scr='.$this->goEditScreen.$callParams.'" title="&nbsp; &nbsp; '.gettext("Go to edit the data").' &nbsp; &nbsp;">
		 </a></div>';
	}

    return $this->inlinePreContent . str_replace('/*content*/', $this->_simpleRender($values), $idDiv) . $this->inlinePostContent.$edit_link ;
}
...
function EditField(&$element, $moduleID)
{
	...
	$this->FromTo = true;
	//if( strtolower( $element->getAttr('FromTo') ) == 'no' ) $this->FromTo = false;	
	if( strtolower( $element->getAttr('FromTo') ) == 'no' AND (
		$moduleField->dataType == 'int' OR 
		$moduleField->dataType == 'float' OR 
		$moduleField->dataType == 'money') ) $this->FromTo = false;	
	...
	switch( $this->dataType ){
		case 'money':
			...
			if( !empty($value) ){
				if( strpos( (string) $nVal, "." ) == false 
					AND strpos( (string) $nVal, "," ) == false ){
					$value = number_format( $value, 2, DECIMAL_SEPARATOR, '' );
				}else{ 
					//there are more than 2 decimals so display the whole thing
					$value = number_format($value, 4, DECIMAL_SEPARATOR, '');
				}			
				$value = str_replace( '.', DECIMAL_SEPARATOR, $value );
			}			
			break;
...
function render(&$values, &$phrases)
{
    ...
    case 'search':
        if( $this->useFromToFields() AND $this->FromTo ){
			$content = '<span class="frmt">&nbsp;&nbsp;&nbsp;'.gettext('from').'</span>';
            $content .= $this->simpleRender($values, $this->name.'_f');
            $content .= '<span class="frmt"> ... &nbsp;'.gettext('to').'</span>';
...
function getSearchPhrase(&$data, &$moduleFields)
{
    ...
    if( $this->useFromToFields() ) {
        ...
        if (!empty($data[$this->name.'_f'])){
            $content .= strip_tags( ShortPhrase($phrase) ). ': &#8807; ' . $data[$this->name.'_f'];
        }
        if (!empty($data[$this->name.'_t'])){
            if(strlen($content) > 0){
                $content .= "<br />\n";
            }
            $content .= strip_tags( ShortPhrase($phrase) ). ': &#8806; ' . $data[$this->name.'_t'];
        }
    ...
    if ($this->checkSearch($data)){
        ...
        if( $this->useFromToFields() ){
			...
			if (!empty($data[$this->name])){
				switch( $this->dataType ){
					case 'int':
						$query = preg_split( "/[\s,;]+/", $data[$this->name] );
						$trimed_query =  array_slice( $query, 0, 100 );
						$clean_query = array();
						$data_query = array();
						foreach( $trimed_query as $query_chunk ){
							array_push( $data_query, $query_chunk );
							$query_chunk = $this->quoteValue( $query_chunk );							
							array_push( $clean_query, $query_chunk );
						}
						$data[$this->name] = implode( ', ', $data_query);
						if( !empty( $data[$this->name] ) ){
							$value = implode( ', ', $clean_query);
							$where[$this->name] = "$name IN ( $value )";
					...
					case 'float':
						...
						foreach( $trimed_query as $query_chunk ){
							array_push( $data_query, $query_chunk );
							$query_chunk = str_replace( DECIMAL_SEPARATOR, '.',  $query_chunk );
							$query_chunk = $this->quoteValue( $query_chunk );							
							array_push( $clean_query, $query_chunk );
						}
						$data[$this->name] = implode( ', ', $data_query);
...
function &Factory($element, $moduleID)
{
    return new PasswordField($element, $moduleID);
}
...
function PasswordField(&$element, $moduleID)
{
	...
	$this->openText        = $element->getAttr('openText');
	if( empty($this->openText) ){
        $this->openText  = false;
    }else{
		$this->openText  = true;
	}
...
function simpleRender(&$values, $overrideName = '')
{
	...
	$content .= '<span class="pwdc">'.gettext("(enter new password)") . "</span>\n";
    ...
    if( $this->confirm ){
        ...
        $content .= '<span class="pwdc">'.gettext("(confirm new password)") . "</span>\n";
    }
...
function &Factory($element, $moduleID)
{
    return new UploadField($element, $moduleID);
}
...
function &Factory($element, $moduleID)
{
    return new DateField($element, $moduleID);
}
...
function simpleRender(&$values, $overrideName = '')
{
	...
	switch( $this->dataType ){
		case'datetime': 
			$size = '22';
			$maxlength = '19';
			$formatString = $User->getDateTimeFormatPHP();
			break;
		case 'date':
			$size = '12';
			$maxlength = '10';
			$formatString = $User->getDateFormatPHP();
			break;
		 case 'time':
			$size = '12';
			$maxlength = '10';
			$formatString = $User->getTimeFormatPHP();
			break;
		default:
			$size = '22';
			$maxlength = '19';
   ...
   //check for nulls and empty values
    if( ((null == $value) || ('0000-00-00' == $value)) AND !$existing ){
        switch( $this->defaultValue ){
        case 'today':
        case 'now':
		...
		switch( $this->dataType ){
			case 'time':
				return $this->inlinePreContent.' '.
					...
					.' '.$this->inlinePostContent;
			case'datetime':
				$value=date_format( date_create($value), 'Y-m-d\TH:i\Z' );
				return $this->inlinePreContent.' '.
					sprintf(
						FORM_DATETIME_HTML_MBL,
						$name,
						$value,
						$size,
						$maxlength,
...
function checkSearch(&$data)
{
    ...
    if( '' != trim($value_from) || '' != trim($value_to) || '' != trim($value) ){
        return true;
    } else {
        return false;
    }
...
function getSearchPhrase(&$data, &$moduleFields)
{
    ...
    if ( !empty($data[$this->name.'_f']) ){
        $content .= strip_tags( ShortPhrase($phrase) ). ' '.gettext("on or after").' ' . $data[$this->name.'_f'];
    }
    if ( !empty($data[$this->name.'_t']) ){
        if(strlen($content) > 0){
            $content .= "<br />\n";
        }
        $content .= strip_tags( ShortPhrase($phrase) ). ' '.gettext("on or before").' ' . $data[$this->name.'_t'];
    }
	if ( !empty($data[$this->name]) ){
        if(strlen($content) > 0){
            $content .= "<br />\n";
        }
        $content .= strip_tags( ShortPhrase($phrase) ). ' '.gettext("on").' ' . $data[$this->name];
    }
    return $content;
}
...
function &Factory($element, $moduleID)
{
    return new TimeField($element, $moduleID);
}
...
function simpleRender(&$values, $overrideName = '')
{
    ...
    if( $values[$this->name] != '' ){
        ...
        if( $mod < $fudge ){ //there are max 2 (non-zero) decimals
            ...
            $content = number_format( $val, 2, DECIMAL_SEPARATOR, '' );
        }else{ //there are more than 2 decimals so display the whole thing
            //print "4 decimals<br>";
            $content = number_format( $val, 4, DECIMAL_SEPARATOR, '' );
        }
    }else{
        $val = '';
    }
	...
	if( !$User->Client['is_Mobile'] ){
		return $this->inlinePreContent.' '.
			...
			.' &nbsp;'.MASTER_CURRENCY.' '.$this->inlinePostContent;
	}else{
		return $this->inlinePreContent.' '.
			sprintf(
				FORM_EDIT_HTML_NUMBER_MBL,
...
function CheckBoxField(&$element, $moduleID)
{
	...
	$this->onlyTrue = $element->getAttr('onlyTrue');
    if( $this->onlyTrue == 'yes' ){
        $this->onlyTrue = true;
    }else{
		$this->onlyTrue = false;
	}
...
function getSearchPhrase(&$data, &$moduleFields)
{
    ...
    $content .= strip_tags( ShortPhrase($phrase) ). ': ' . $valuePhrase;
    return $content;
}
...
function MemoField(&$element, $moduleID)
{
	...
	$this->defaultValue = $element->getAttr('defaultText'); 
...
function ComboField(&$element, $moduleID)
{
	...
	$optionSpace = $element->getAttr('optionSpace');
    if( $optionSpace == 'yes' ){
        $this->optionSpace = true;
    }else{
		$this->optionSpace = false;
	}
    ...
    $this->phrase = $element->getAttr('phrase');
    if(empty($this->phrase)){
        $this->phrase = $list_moduleField->phrase;
    }
    $this->inlinePreContent  = $this->_inlineFormat($element->getAttr('inlinePreContent'));
    $this->inlinePostContent = $this->_inlineFormat($element->getAttr('inlinePostContent'));
    ...
    if('org' == $this->foreignTable){
        trigger_error("The ComboField {$this->name} in the module {$moduleID} uses the 'org' (Organizations) module as its list module. Although that will work fine in many cases, the Organizations-specific feature orgListOptions is not supported in the ComboField class. It is recommended that you change this element into an OrgComboField.", E_USER_NOTICE);
    }
...
function _buildSQLwChildDep(){
    ...
    foreach($this->childFields as $childFieldName => $childFieldAttrs){
        if(isset($childFieldAttrs['listParentField'])){
            ...
            $this->getSQL = $SQL . "\nWHERE $idField = '/*recordID*/' AND `{$childListForeignModuleID}`._Deleted = 0\n";
...
//print_r($listFields);
    ...
    $this->getSQL = $SQL . "\nWHERE $idField = '/*recordID*/' AND `{$listModuleID}`._Deleted = 0\n";
    CheckSQL($this->getSQL);
...
function getAddNewLink()
{
    ...
    if(!$this->suppressItemAdd && 'search' != $this->renderMode){
        $addNewLink = "<a href=\"#\" onclick=\"window.open('frames_popup.php?dest=edit&amp;mdl={$this->foreignTable}', '{$this->name}AddItem')\">".'<img src="'.$theme_web.'/img/add.png" title="'.gettext("Add new item in new window").'" alt="'.gettext("Add new item in new window").'" />'."</a>";
    }
...
function getListData(&$selected, &$values, $format = null, $isDefault = false){
    ...
    default:
        ...
        $msg = sprintf(gettext("Permission Error:  You have no permission to view records in the %s module"), gettext( $foreignModuleName ) );
...
function getSearchPhrase(&$data, &$moduleFields)
{
    ...
    $content .= strip_tags( ShortPhrase($phrase) ). ': ' . $item;
...
function getSearchCondition(&$data)
{
    ...
    default:
		if( $value < -1000 ){
			# offset 1000 for better everything else than CodeID visibilty
			$value = $this->quoteValue( -($data[$this->name]+1000) );
			return "$name != $value";			
		}else{	
			$value = $this->quoteValue($data[$this->name]);
			return "$name = $value";
...
function getAddNewLink()
{
    ...
    if(!$this->suppressItemAdd && 'search' != $this->renderMode){
        ...
        $addNewLink = "<a href=\"#\" onclick=\"window.open('frames_popup.php?dest=$dest', '{$this->name}AddItem')\">".'<img src="'.$theme_web.'/img/add.png" title="'.gettext("Add new item in new window").'" alt="'.gettext("Add new item in new window").'" />'."</a>";
    }
...
function simpleRender(&$values)
{
    ...
    if( !empty($values[$this->name]) ){
		...
		if( !$existing ){
			$value = $this->getDefaultValue($values);
			if( empty( $value ) ){
				$orgvalue = $this->orgField->getDefaultValue();
				if( !empty( $orgvalue ) ){
					$orgDefaultExists = true;
				}else{
					$orgDefaultExists = false;
				}
			}
    ...
    if(count($this->listConditions) > 0){
        foreach($this->listConditions as $listCondition){
            $exprArray = $listCondition->getExpression('ppl');
            $wheres[] = $exprArray['expression'];
        }
    }
	if( count($wheres) > 0 ){
	...
	if( count($wheres) > 0 ){
		$SQL = 'SELECT PersonID, DisplayName, OrganizationID FROM ppl WHERE _Deleted = 0 '
		.$wheresSQL.' ORDER BY DisplayName';
	}else{
    ...
    $jsCode2 = '';
	$i = 0;
	$OrgsWithPersons = array( -2 => true, -1 => true,  0 => true );
    foreach($r as $row){
        $jsCode2 .= "\tar{$this->name}People[{$i}] = new Array({$row[0]}, \"".addslashes($row[1])."\", {$row[2]});\n";
        $i++;
		$OrgsWithPersons[ $row[2] ] = true;
    }
    ...
    foreach($orgList as $orgID => $row){
		if( array_key_exists( $orgID, $OrgsWithPersons ) ){
			$jsCode1 .= "\tar{$this->name}Orgs[{$i}] =  new Array({$orgID}, \"{$row[0]}\");\n";
			$i++;
		}
    }
...
function getListData($selected, &$values, $format = null)
{
    ...
    foreach( $r as $id=>$name ){
		...
		$name = str_replace( '!themeDirectory!', $theme_web, $name );
...
function CodeRadioField(&$element, $moduleID)
{
    ...
    if( 'yes' == $element->getAttr('bool') ){
		...
		if( $element->getAttr('onlyTrue') ){
			$this->SQL = "SELECT CodeID AS ID, Description AS NAME FROM cod\n";
			$this->SQL .= " WHERE _Deleted = 0 AND cod.CodeTypeID = 11\n";
...
function getSearchPhrase(&$selectedValues)
{
    ...
    return strip_tags( ShortPhrase($phrase) ). ': '.join(', ', $labels);
}
