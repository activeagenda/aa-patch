...
trace($SQL, $debug_prefix.' SQL');
    ...
    $r = $mdb2->exec($SQL);
    $mdb2error = mdb2ErrorCheck($r);	
	if( isset( $mdb2error['code'] ) AND $mdb2error['code'] != 0 ) return $mdb2error;
...
//              trace($sql, 'DataHandler verifyUniqueness '.$this->moduleID);
                ...
                if(count($verifyResult) > 1){
                    $msg = "";
                    trigger_error(gettext("Warning: The uniqueness index ").$ixName.gettext(" (fields: ").join(',',array_keys($ixFields)).") of the ".$this->moduleID." module is violated. Please clean the data.", E_USER_WARNING);
                }
                    ...
                    if(count($verifyResult) > 0){
                        ...
                        if(!$matched){
                            ...
                            foreach(array_keys($ixFields) as $fieldName){
                                $moduleIxField = $moduleFields[$fieldName];
								$msgValues[] = ShortPhrase( gettext($moduleIxField->phrase) );
                            }
                            $moduleInfo = GetModuleInfo($this->moduleID);
                            $moduleName = gettext( $moduleInfo->getProperty('moduleName') );
                            $errmsg = array(sprintf(gettext("The supplied data (%s) matches a different record in the %s module. To save this record, you must choose a different value."), join(', ',$msgValues), $moduleName) => 'uniq02');
                            $this->errmsg = array_merge($errmsg, (array)$this->errmsg);
                            return false;
                        }
                    }
...
trace($verifyResult, 'verifyResult: we expected empty');
						...
						$moduleInfo = GetModuleInfo($this->moduleID);
						$moduleName = gettext( $moduleInfo->getProperty('moduleName') );
                        $errmsg = array(sprintf(gettext('The supplied data (%s) matches an existing record in %s.'), join(', ',$msgValues), $moduleName ) => 'uniq03');
						$this->errmsg = array_merge($errmsg, (array)$this->errmsg);
                        return false;
                    }
                }
...
trace($exists, $this->moduleID .' record exists');
    ...
    if($exists){
        //update record
        $mdb2error = $this->_update($values);		
		if( isset( $mdb2error['code'] ) AND $mdb2error['code'] != 0 ) return $mdb2error;
    } else {
        //insert record
        $mdb2error = $this->_insert($values);
		if( isset( $mdb2error['code'] ) AND $mdb2error['code'] != 0 ) return $mdb2error;
    }
