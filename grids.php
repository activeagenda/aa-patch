...
function ViewGrid($element, $moduleID)
{
	...
	$this->rowIDon = $element->getAttr('rowIDon');
	$this->moduleOn = $element->getAttr('moduleOn');
	$this->perPage = $element->getAttr('perPage');
	$this->goEditScreen = $element->getAttr('goEditScreen');
	$this->goSelectScreen = $element->getAttr('goSelectScreen');
	$this->name = $element->getAttr('name');
	
	$this->akaID = $element->getAttr('akaID');
	if( !empty($this->akaID) ){
        $this->formatOptions['akaID'] = $this->akaID;
        $this->isModuleAlias = true;
    }else{
...
function render($page, $qsArgs)
{
	...
	if( ! $User->PermissionToView( $this->moduleID ) ){
        ...
        $content = sprintf(
            VIEWGRID_MAIN,
            $gridTitle,
            '<br/>&nbsp;&nbsp;&nbsp;'.sprintf(gettext("You have no permissions to view records in the %s module."), '"'.gettext($moduleName).'"' ) . "&nbsp;&nbsp;&nbsp;<br/>&nbsp;\n",
            ''
			);
    ...
    if( ( !isset($this->formatOptions['suppressPaging']) || !$this->formatOptions['suppressTitle'] )
		 && ( !defined('IS_RPC') || !IS_RPC ) ){
        if(empty($nRows)){
            $count = '';
        } else {
            $count = '( '.$nRows.' )';
        ...
        if( !isset($this->formatOptions['infoTitle']) || !$this->formatOptions['infoTitle'] ){
			$filterParams = null;
			if( isset( $this->moduleOn ) ){
				$filterParams .= '&amp;'.$this->moduleOn.'='.$qsArgs['mdl'];
			}			
			if( isset( $this->rowIDon ) ){
				$filterParams .= '&amp;'.$this->rowIDon.'='.$qsArgs['rid'];
			}
			...
			if( isset( $this->name ) ){
				$gridAnchor = ' id="'.$this->name.'"';
			}			
			$callingModuleID = $qsArgs['mdl'];
			
			if( $this->isModuleAlias ){
				$gridModuleID = $this->akaID;
			}else{
			...
			( $User->PermissionToEdit( $callingModuleID )>0 OR !empty( $EditScrPermission[$this->goEditScreen] ) )  ){
				...
				$gridTitle = sprintf(
					GRID_TAB_EGO,
					gettext($this->phrase),
					'list.php?mdl='.$gridModuleID.$filterParams,
					'frames_popup.php?dest=list&amp;mdl='.$gridModuleID.$filterParams,
					$count,
					'edit.php?scr='.$this->goEditScreen.$callParams.'#enfl',
					$gridAnchor
					);
			}elseif( isset( $this->goSelectScreen ) AND $User->PermissionToEdit( $qsArgs['mdl'] )>0 ){
				$callParams = $qsArgs;
				unset( $callParams['scr'] );
				$callParams = '&amp;'.MakeQS( $callParams );
				$gridTitle = sprintf(
					...
					'frames_popup.php?dest=list&amp;mdl='.$gridModuleID.$filterParams,
					$count,
					'edit.php?scr='.$this->goSelectScreen.$callParams.'#enfl',
					$gridAnchor
					);						
			}else{
				if( in_array( $this->moduleID, array( 'nts', 'lnk', 'cos', 'rmd' ) ) ){
					$callParams = $qsArgs;
					unset( $callParams['scr'] );
					unset( $callParams['gmd'] );
					$callParams = '&amp;'.MakeQS( $callParams );
					$gridTitle = sprintf(
						GRID_TAB_GGO,
						gettext($this->phrase),
						...
						'frames_popup.php?dest=list&amp;mdl='.$gridModuleID.$filterParams,
						$count,
						'global.php?gmd='.$gridModuleID.$callParams.'#enfl',
						null
						);
				}elseif( $this->moduleID == 'att' AND $User->PermissionToEdit( $qsArgs['mdl'] )>0 ){
					$callParams = $qsArgs;
					unset( $callParams['scr'] );
					unset( $callParams['gmd'] );
					$callParams = '&amp;'.MakeQS( $callParams );
					$gridTitle = sprintf(
						...
						'frames_popup.php?dest=list&amp;mdl='.$gridModuleID.$filterParams,
						$count,
						'edit.php?scr=Attachments'.$callParams.'#enfl',
						null
						); 
				}else{
					$gridTitle = sprintf(
            ...
            foreach($this->Fields as $field){
                if($field->isVisible() && empty($field->parentName)){
                    if(array_key_exists($field->name, $this->verticalFormats)){
                        ...
                        $rowContent .= sprintf(
                            $formatStr,
                            ShortPhrase( gettext($field->phrase) ),
                            $row[$field->name]
                            );
                    }
                }
...
var $addRecordOnly = false;
var $quickEditAdd = false;
var $quickEditOnly = false;
...
function &Factory($element, $moduleID)
{
	...
	if( 'yes' == strtolower( $element->getAttr('addRecordOnly') ) ){
        $grid->addRecordOnly = true;
        $grid->formatOptions['suppressRecordIcons'] = true;
    }	
	
	if('yes' == strtolower( $element->getAttr('quickEditOnly') )){
        $grid->quickEditOnly = true;
    }
	if('yes' == strtolower( $element->getAttr('quickEditAdd') )){
        $grid->quickEditAdd = true;		
    }
	
	$grid->allowEditOn = $element->getAttr('allowEditOn');
	if( !empty( $grid->allowEditOn ) ){        
        $grid->formatOptions['allowRecordIconsOn'] = $grid->allowEditOn;
    }
	
	$grid->rowIDon = $element->getAttr('rowIDon');
	$grid->moduleOn = $element->getAttr('moduleOn');
	$grid->perPage = $element->getAttr('perPage');
	$grid->goViewAnchor = $element->getAttr('goViewAnchor');
            ...
            if('GridForm' == $sub_element->type){
                foreach($sub_element->c as $form_element){
                    ...
                    default:
						...
						if( !empty( $form_element->attributes['phrase'] ) ){
							$field_object->phrase = $form_element->attributes['phrase'];
						}else{
							switch( strtolower( get_class( $field_object ) ) ){
								case 'combofield':
									if( !empty( $form_element->attributes['listField'] ) ){
										$listSourceFieldName = $form_element->attributes['listField'];
									}else{
										$listSourceFieldName = substr( $field_object->name, 0, -2 ); 
									}
									break;
								default:
									$listSourceFieldName = $field_object->name;
							}
							$field_object->phrase = $subModule->ModuleFields[$listSourceFieldName]->phrase;
						}
...
function render($page, $qsArgs)
{
    ...
    if(! $User->PermissionToView($this->moduleID) ){
        ...
        $content = sprintf(
            VIEWGRID_MAIN,
            $gridTitle,
            '&nbsp;<br/>&nbsp;&nbsp;&nbsp;'.sprintf(gettext("You have no permissions to view or edit records in the %s module."), '"'.gettext($moduleName).'"') . "&nbsp;&nbsp;&nbsp;<br/>&nbsp;\n",
            ''
			);
        return $content;
    }
    ...
    if($this->listExtended){
        ...
        if( true == $this->allowAddRecord ){
            $gridType = 'edit';
        } else {
            $gridType = 'edit_noadd';
        }
		if( true == $this->quickEditOnly ){
			$gridType = 'edit_nfe';
		}
		if( true == $this->quickEditAdd ){
			$gridType = 'edit_aqe';
		}
    ...
    $gridform_template = 
    '<div id="gridFormDiv_%s" style="position:absolute;display:none;z-index:3;" class="l3"><table class="frm">%s</table></div>';

    $formDiv = sprintf($gridform_template, $this->moduleID, '<tr><td></td></tr>');
    //$content = sprintf(VIEWGRID_TABLE, $content);
	...
	if( isset( $qsArgs['iVlp'] ) ){
		if( isset( $this->goViewAnchor ) ){					
			$callParams = $qsArgs;
			unset( $callParams['scr'] );
			$callParams = MakeQS( $callParams );
			$gridTitle = sprintf(
				GRID_TAB_VGO,
				gettext($this->phrase),
				'list.php?mdl='.$this->moduleID.$filterParams,
				'frames_popup.php?dest=list&amp;mdl='.$this->moduleID.$filterParams,
				$count,
				'view.php?'.$callParams
				);
		}else{	
		...
		if( isset( $this->goViewAnchor ) ){		
			$callParams = $qsArgs;
			unset( $callParams['scr'] );
			$callParams = MakeQS( $callParams ).'#'.$this->goViewAnchor;
			$gridTitle = sprintf(
				GRID_TAB_VGO,
				gettext($this->phrase),
				'list.php?mdl='.$this->moduleID.$filterParams,
			...
			if( in_array( $this->moduleID, array( 'nts', 'lnk', 'cos','rmd' ) ) ){
				$callParams = $qsArgs;
				unset( $callParams['scr'] );
				$callParams = MakeQS( $callParams ).'#Global';
				$gridTitle = sprintf(
					GRID_TAB_VGGO,
					gettext($this->phrase),
					'list.php?mdl='.$this->moduleID.$filterParams,
    ...
    if( !defined('IS_RPC') || !IS_RPC ){
		...
		if( in_array( $this->moduleID, array( 'nts', 'lnk', 'cos','rmd' ) ) ){
			$globalDivClass = ' class="glblst" ';
			$globalIDinner = ' id="glbinnr" ';
		}		
        $content = '<div'.$globalDivClass.' id="list_'.$this->moduleID.'">'.$content.'</div>';
        $content = sprintf(
            VIEWGRID_MAIN,
            $gridTitle,
            $content,
    ...
    $content .= $formDiv;
    $content .= "\n<script id=\"{$this->moduleID}\" type=\"text/javascript\" >\n";	
    $content .= "editGrids['{$this->moduleID}'] = {";
    $content .= "recordID: $recordID, ";
    $content .= "moduleID: '$ModuleID', ";
    $content .= "useRowClassNum: 1, ";
    $content .= "rowClasses: Array('l', 'l2')}; ";
    $content .= "\n</script>\n";
    return  $content;
}
...
trace($SQL, 'getFormSQL');
        ...
        foreach( $fieldList as $fName => $field ){
            ...
            if( 'datefield' == strtolower(get_class($field)) ){
				...
				$datecontent .= "   onUpdate    : indicateUnsavedDateChanges,\n";
    ...
    $content .= sprintf( FORM_BUTTONROW_HTML,
        sprintf( FORM_BUTTON_SUBMIT_HTML, 'Save', gettext("Save"), 'editGridSaveForm(moduleID, submoduleID, editRowID)' ).' '.
        $deleteButton.
	...
	$postcontent = "<script type=\"text/javascript\">\n<!--\n//[js:]\n";
	$postcontent .= "setFocusOnFirstInputInForm('{$this->moduleID}');\n";    
	$postcontent .= "//[:js]\n-->\n</script>\n";
...
function validateForm( $ownerField = null )
{
	global $User;
    ...
    foreach($fields as $fieldname => $field){
		if( isset( $ownerField ) ){
			if( $fieldname != $ownerField ) continue;
        }
		$value = '';
        if( isset($_POST[$fieldname]) ){
           
			if( $field->dataType == 'float' 
				or $field->dataType == 'money' 
				or $field->dataType == 'int' ){
				if( !empty($_POST[$fieldname]) ){
					$_POST[$fieldname] = str_replace( ' ', '', $_POST[$fieldname] );
					$_POST[$fieldname]= str_replace( DECIMAL_SEPARATOR, '.', $_POST[$fieldname] );
				}
			}
			...
			if( $field->dataType == 'datetime' ){
				if( !empty($_POST[$fieldname]) ){
					...
					$timestamp = date_create_from_format('d.m.y G:i', $_POST[$fieldname] );
					if( $timestamp === false ){
						$timestamp = date_create_from_format('d.m.y G.i', $_POST[$fieldname] );
					}	
					if( $timestamp === false ){
						$timestamp = date_create_from_format('d.m.y G', $_POST[$fieldname] );
					}
					if( $timestamp === false ){
						$timestamp = date_create_from_format('y-m-d G', $_POST[$fieldname] );
					}
					if( $timestamp === false ){
						$timestamp = date_create_from_format('Y-m-d G', $_POST[$fieldname] );
					}
					if( $timestamp === false ){
						$timestamp = date_create( $_POST[$fieldname] );
					}	
					if( $timestamp === false ){
						//$_POST[$fieldname] = $_POST[$fieldname];
					}else{
						//$_POST[$fieldname] = date_format( $timestamp, "Y-m-d H:i" );
						...
						$_POST[$fieldname] = date_format( $timestamp, $dateformat );
					}
				}
			}
			...
			if( $field->dataType == 'date' ){
				if( !empty($_POST[$fieldname]) ){
					$_POST[$fieldname] = str_replace( ' ', '', $_POST[$fieldname] );
					$timestamp = date_create_from_format('d.m.y', $_POST[$fieldname] );
					if( $timestamp === false ){
						$timestamp = date_create( $_POST[$fieldname] );
					}
					if( $timestamp === false ){
						...
						//$_POST[$fieldname] = date_format( $timestamp, 'Y-m-d' );
						$_POST[$fieldname] = date_format( $timestamp, str_replace( "%", '', $User->getDateFormatPHP()) );
					}
				}
			}
			...
			if( $field->dataType == 'time' ){
				if( !empty($_POST[$fieldname]) ){
					$_POST[$fieldname] = str_replace( ' ', '', $_POST[$fieldname] );
					$timestamp = date_create_from_format('G', $_POST[$fieldname] );
					if( $timestamp === false ){
						$timestamp = date_create( $_POST[$fieldname] );
					}
					if( $timestamp === false ){
						...
						if( $interval->days > 0 OR $timestamp->getOffset() != $midnight->getOffset() ){
							...
							//$_POST[$fieldname] = date_format( $timestamp, 'H:i' );
							$dateformat = str_replace( "%", '', $User->getTimeFormatPHP() );
							$dateformat = str_replace( "I", 'g', $dateformat  );
							$dateformat = str_replace( "M", 'i', $dateformat  );
							$dateformat = str_replace( "p", 'a', $dateformat  );
							$_POST[$fieldname] = date_format( $timestamp, $dateformat );
        ...
        $message = Validate($_POST, $value, ShortPhrase( gettext($field->phrase) ), $field->validate, $field->dataType);
        $field->invalid = true;        
//START    
...
trace($dh->relatedRecordFieldValues, "EditGrid-dh relatedRecordFieldValues");
    ...
    if('save' == $action){
        ...
        }elseif( is_array( $resultID ) AND isset( $resultID['code'] ) ){
			$error .= gettext("The record has not been saved, because:")."\n- ".$resultID['native_code']."\n".' ';
		}
		...
		$validateMsg = $this->validateForm( '_OwnedBy' );
        if(!empty($validateMsg)){
            $error .= gettext("The data was not deleted, because:")."\n".$validateMsg."\n".' ';
		...
		if( is_array( $success ) AND isset($success['code']) ){
			$error .= gettext("The record has not been deleted, because:")."\n- ".$success['native_code']."\n".' ';
		}
...
//print debug_r($this->Fields);
    ...
    foreach( $r as $rowNum => $row ){
		...
		if( isset( $this->allowEditOn ) ){			
			if( isset($row[$this->allowEditOn]) ){
				if( $row[$this->allowEditOn] == $_SESSION['User']->PersonID OR $_SESSION['User']->IsAdmin ){
					$showRowLinks = true;
				}else{
					$showRowLinks = false;
				}
...
//DEV
    ...
    //format grid table
	$pagefromQS = $page.'?'.$formQS;
	if( $this->selectedID == 0 ) $pagefromQS = $page.'?'.$rowQS;
    ...
    if (empty($_POST['cancel'])){
        ...
        if (intval($_POST['gridnum']) == $this->number){
            ...
            if(! $User->PermissionToEdit($this->moduleID) ){
                $moduleInfo = GetModuleInfo($this->moduleID);
                $moduleName = $moduleInfo->getProperty('moduleName');
                trigger_error(sprintf(gettext("You have no permissions to edit records in the %s module."), gettext($moduleName)) . E_USER_ERROR);
            }
...
//trace($_GET, 'GET');
			...
			if( !isset( $_POST['save'] ) AND !isset( $_POST['add'] ) AND  !isset( $_POST['delete'] ) ){
            ...
            if(!empty($_POST['delete'])){
				...
				if(!empty($validateMsg)){
					...
					$validateMsg = gettext("The data was not deleted, because:")."\n".$validateMsg.' ';
                ...
                case UPLOAD_ERR_OK:
                    if(is_uploaded_file($_FILES['FileName']['tmp_name'])){
                        ...
                        if(empty($_POST['Description'])){
							...
							$uploadinfo = pathinfo( $uploadfilename );
							$uploaddescription = basename( $uploadfilename, '.'.$uploadinfo['extension'] );
							$uploaddescription = str_replace( '_', ' ', $uploaddescription );
							$uploaddescription = str_replace( '-', ' ', $uploaddescription );
                            $_POST['Description'] = $uploaddescription;
                        }
                ...
                case UPLOAD_ERR_NO_FILE:                    
					if (!empty($_POST['add'])){
                        ...
                        $rowID = $dh->saveRow($_POST, $rowID);
                        $this->selectedID = 0;
						//Bypassing the style flblh caused by $field->invalid = true from valiadateForm
						if($this->hasGridForm){
							$fields =& $this->FormFields;
						} else {
							$fields =& $this->Fields;
...
function SelectGrid (&$element, $moduleID)
{
	...
	$this->rowIDon = $element->getAttr('rowIDon');
	$this->moduleOn = $element->getAttr('moduleOn');
	$this->goViewAnchor = $element->getAttr('goViewAnchor');
...
function handleForm(){
    if(!empty($_POST)){
        ...
        if(! $User->PermissionToEdit($this->moduleID) ){
            $moduleInfo = GetModuleInfo($this->moduleID);
            $moduleName = $moduleInfo->getProperty('moduleName');
            die(sprintf(gettext("You have no permissions to edit records in the %s module."), gettext($moduleName)) . "<br />\n");
        }
        ...
        if(!empty($_POST['SaveIDs'])){
            ...
            $uncleanDataArray = preg_split('/ /', trim($_POST['SaveIDs']));
