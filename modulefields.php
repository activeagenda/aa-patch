...
//var $deleteKeys;					//for handling denormalization over triggers	
...
function TableField(&$element, $moduleID)
{
	...
	//START	
	$this->deleteKeys = $element->getAttr('deleteKeys');	
	if ( !empty( $this->deleteKeys ) ){
		$this->deleteKeys = trim( $this->deleteKeys );
		$this->deleteKeys = preg_split( '/\s+/', $this->deleteKeys );			
	}
	$this->deleteOnIdGlobals = $element->getAttr('deleteOnIdGlobals');
	if ( !empty( $this->deleteOnIdGlobals ) ){
		$this->deleteOnIdGlobals = trim( $this->deleteOnIdGlobals );
		$this->deleteOnIdGlobals = preg_split( '/\s+/', $this->deleteOnIdGlobals );
	}
	//xt._TaskStatusID/_ProjectID=xt._ProjectID
	$this->passValueOn = $element->getAttr('passValueOn');
	if ( !empty( $this->passValueOn ) ){
		$this->passValueOn = trim( $this->passValueOn );
		$this->passValueOn = preg_replace( '/\s+/', ' ', $this->passValueOn );
		$this->passValueOn = preg_split( "/\s+\|\s+/", $this->passValueOn );		
	}
//END
...
function &Factory($element, $moduleID)
{
    return new ForeignField($element, $moduleID);
}
...
function makeJoinDef($pModuleID)
    {
        ...
        if($dt_localKeyField == 'foreignfield' && $localKeyField->getForeignFieldType() != 'tablefield'){
            ...
            if('tablefield' == $dt_localKey_ForeignField){
                ...
                case 'remotefield':
                    ...
                    if (!empty($this->listCondition)){
                        $bridgeJoinConditions[] = "`{$foreignAlias}`.{$this->listCondition} AND `{$foreignAlias}`._Deleted = 0";
                    }
            ...
            $join .= "\n   ON ($localKeyName = $foreignKeyName AND `{$foreignAlias}`._Deleted = 0";
            ...
            if(!empty($this->localKey2)){
                ...
                $join .= "\n     AND $localKeyName2 = $foreignKeyName2 AND `{$foreignAlias}`._Deleted = 0";
            }
            ...
            if (!empty($this->triggerCondition)){
                ...
                $join .= "\n     AND `{$foreignAlias}`.$condition AND `{$foreignAlias}`._Deleted = 0";
            }
...
function &Factory($element, $moduleID)
{
    return new CodeField($element, $moduleID);
}
...
function &Factory($element, $moduleID)
{
    return new CombinedField($element, $moduleID);
}
    ...
    foreach($this->content as $content){
        if (is_array($content)){
            ...
            if(0 < strlen(trim($content))){
                ...
                $staticContent .= str_replace('\'', '', $translatedContent);
            }
        }
    }
...
function &Factory($element, $moduleID)
{
    return new DynamicForeignField($element, $moduleID);
}
...
function &Factory($element, $moduleID)
{
    return new CalculatedField($element, $moduleID);
}
...
function CalculatedField(&$element, $moduleID)
{
	...
	$params = preg_split( '/\s+/', $element->getAttr('params') );
...
function makeSelectDef($pModuleID, $pIncludeFieldAlias = true, $localizeOutput = true)
{
	...
	case 'unixtimestamp':
        $this->checkParameters(1, 1);
        $content = "UNIX_TIMESTAMP({$params_translated[0]})";
        break;	
    case 'filesize':
        $this->checkParameters(1, 1);
	...
	case 'digest':
		$this->checkParameters(2, 2);
		$content = "CASE WHEN CHAR_LENGTH({$params_translated[0]}) > {$params_translated[1]} THEN CONCAT(SUBSTRING( {$params_translated[0]} FROM 1 FOR {$params_translated[1]}), '...') ELSE {$params_translated[0]} END";
		break;
    case 'duedateformat':
        $this->checkParameters(1, 1);
...
//DEV	
	case 'copy_field':
        $this->checkParameters(1, 1);
        $content = $params_translated[0];
        break;	
	case 'att_url':  //returns download from att link for pictures
        $this->checkParameters(3, 3);
        $content = "CONCAT( 'download.php?pmb=1&mdl=',{$params_translated[0]}, '&rid=', {$params_translated[1]}, '&fid=', {$params_translated[2]}  )";
        break;
	case 'img_from_att':  //returns download from att link for pictures
        $this->checkParameters(5, 5);
        $content = "CONCAT( '<img class=\\\"img_from_att\\\" style=\\\"max-width:{$params_translated[3]}px;max-height:{$params_translated[4]}px;\\\" src=\\\"download.php?pmb=1&mdl=',{$params_translated[0]}, '&rid=', {$params_translated[1]}, '&fid=', {$params_translated[2]},'\\\"\/>'  )";
        break;	
    case 'item_exists':  //returns download from att link for pictures
        $this->checkParameters(1, 1);
        $content = "CONCAT( '<div class=\\\"item_exists\\\" title=\\\"', {$params_translated[0]}, '\\\">&#9609;&#9609;</div>'  )";
        break;
	case 'flipflop':  //returns sign according to true or false
        $this->checkParameters(3, 3);
        $content = "CASE WHEN {$params_translated[0]} = 1 THEN '{$params_translated[1]}' ELSE '{$params_translated[2]}' END";
        break;
	case 'flipflop_onnull':  //returns sign according to true or false
        $this->checkParameters(3, 3);
        $content = "CASE WHEN {$params_translated[0]} IS NULL THEN '{$params_translated[2]}' ELSE '{$params_translated[1]}' END";
        break;
	case 'flipflop_onone':  //returns sign according to true or false
        $this->checkParameters(2, 2);
        $content = "CASE WHEN {$params_translated[0]} = 1 THEN '{$params_translated[1]}' ELSE NULL END";
        break;
	case 'repeat_sign':
        $this->checkParameters(2, 2);
        $content = "REPEAT('{$params_translated[1]}', {$params_translated[0]})";
        break;
	case 'point_in_timeinterval':
        $this->checkParameters(3, 3);
        $content = "( {$params_translated[0]} <= DATE_ADD(CURDATE(), INTERVAL {$params_translated[2]} DAY) AND DATE_ADD(CURDATE(), INTERVAL {$params_translated[2]} DAY) <= {$params_translated[1]} )";
        break;
	case 'week_in_timeinterval':
        $this->checkParameters(4, 4);
        $content = 
		"( {$params_translated[0]} < DATE_ADD(CURDATE(), INTERVAL ( (7-DAYOFWEEK(CURDATE()))+ {$params_translated[3]} ) DAY) AND DATE_ADD(CURDATE(), INTERVAL ( (1-DAYOFWEEK(CURDATE()))+ {$params_translated[2]} ) DAY) < {$params_translated[1]} )";
        break;
	case 'if_concat':
		$this->checkParameters(2, 2);
        $content = "CASE WHEN {$params_translated[0]} IS NULL THEN NULL ELSE CONCAT( {$params_translated[0]}, {$params_translated[1]} ) END";
        break;
	case 'date_outside_interval':
        $this->checkParameters(3, 3);
        $content = "( {$params_translated[0]} < {$params_translated[1]} OR {$params_translated[2]} < {$params_translated[0]} )";
        break;
	case 'date_outside_interval12':
        $this->checkParameters(3, 3);
        $content = "CASE WHEN {$params_translated[0]} < {$params_translated[1]} OR {$params_translated[2]} < {$params_translated[0]}  
		OR {$params_translated[1]} IS NULL OR {$params_translated[2]} IS NULL THEN 1 ELSE 2 END";
        break;
	case 'today_outside_interval12':
        $this->checkParameters(2, 2);
        $content = "CASE WHEN CURDATE() < {$params_translated[0]} OR {$params_translated[1]} < CURDATE()  
		OR {$params_translated[0]} IS NULL OR {$params_translated[1]} IS NULL THEN 1 ELSE 2 END";
        break;
	case 'halfhour_logic':
		$this->checkParameters(6, 6);
        $content  = 
		"CASE 
			WHEN {$params_translated[0]} = 0 AND {$params_translated[1]} = 0 THEN '{$params_translated[2]}' 
			WHEN {$params_translated[0]} = 1 AND {$params_translated[1]} = 0 THEN '{$params_translated[3]}' 
			WHEN {$params_translated[0]} = 0 AND {$params_translated[1]} = 1 THEN '{$params_translated[4]}'
			WHEN {$params_translated[0]} = 1 AND {$params_translated[1]} = 1 THEN '{$params_translated[5]}' 			
		END";
        break;
	case 'day_of_week':
        $this->checkParameters(1, 1);
        $content = "DAYOFWEEK({$params_translated[0]})";
        break;	
	case 'not_saturday':
        $this->checkParameters(1, 1);
        $content = "NOT (DAYOFWEEK({$params_translated[0]}) = 7)";
        break;
	case 'not_sunday':
        $this->checkParameters(1, 1);
        $content = "NOT (DAYOFWEEK({$params_translated[0]}) = 1)";
        break;
	case 'calendar_week':
        $this->checkParameters(1, 1);
        $content = "WEEKOFYEAR({$params_translated[0]})";
        break;
	case 'sec_to_hhmm':
        $this->checkParameters(1, 1);
        $content = "TIME_FORMAT( (SEC_TO_TIME({$params_translated[0]})), '%H:%i')";
        break;	
	case 'hhmm_timediff':
        $this->checkParameters(2, 2);
        $content = "TIME_FORMAT( (TIMEDIFF({$params_translated[0]}, {$params_translated[1]})), '%H:%i')";
        break;
	case 'days_to_now':
        $this->checkParameters(1, 1);
        $content = "DATEDIFF(  NOW(), {$params_translated[0]} )";
        break;
	case 'is_day':
		$this->checkParameters(2, 2);
        $content = "CASE WHEN {$params_translated[0]} <= DATE_ADD(CURDATE(), INTERVAL {$params_translated[1]} DAY) 
					AND CURDATE() <= {$params_translated[0]} THEN 1 ELSE 0 END";	
		break;
	case 'account_not_valid':
        $this->checkParameters(3, 3);
        $content = "NOT( {$params_translated[0]} = 0
		AND ( {$params_translated[1]} <= CURDATE() OR {$params_translated[1]} IS NULL ) 
		AND ( CURDATE() <= {$params_translated[2]} OR {$params_translated[2]} IS NULL ) )";
        break;
	case 'same_as2':
		$this->checkParameters(3, 3);
        $content = "( {$params_translated[0]} = {$params_translated[1]} OR {$params_translated[0]} = {$params_translated[2]} )";
        break;				
//END DEV		
	default:
...
function &Factory($element, $moduleID)
{
    return new SummaryField($element, $moduleID);
}
...
function &Factory($element, $moduleID)
{
    return new RangeField($element, $moduleID);
}
...
function &Factory($element, $moduleID)
{
    return new StaticField($element, $moduleID);
}
