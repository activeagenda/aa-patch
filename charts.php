...
if(isset($_GET['dashchart']) && 'set' == $_GET['dashchart']){
    ...
    if(count($search->phrases) > 0){
		$searchPhrases = array_map( 'strip_tags', $search->phrases);
        $phrases = join( '<br />', $searchPhrases );
    } else {
...
$content .= "<br />\n";
$content .= '<div class="searchFilter"><a href="search.php?mdl='.$ModuleID.'" title="&nbsp; &nbsp; '.gettext("Search").' &nbsp; &nbsp;"><img src="'.$theme_web.'/img/search_icon.png"/>&nbsp;<b>'.gettext("Search Filter Conditions").':</b></a><br/><br/>'."\n";     
$content .= str_replace( '!themeDirectory!', $theme_web, $search->getPhrases() );
if( $search->hasConditions() ){			
            $content .= "<br/><br/>\n";                
			$defaultFilterLink = 'defaultFilter=0&amp;';
            $clearSearchLink = '<a href="list.php?clear=1&amp;rdct=charts&amp;'.$defaultFilterLink.'&amp;mdl='.$ModuleID.'">'.gettext("Clear Search Filter (removing conditions)").'</a>';
...
<!--
    ...
    function replaceChart(val){        
        ...
        img_chart.src="chartViewer.php?mdl="+moduleID+"\&chn="+val;
...
if ($ChartDefined){
	...
	<div id="dashChartForm" style="display:none">
		<select id="dashChartPlace" name="dashChartPlace" class="edt">
		<option value="top">'.gettext("Top").'</option>
		<option value="bottom">'.gettext("Bottom").'</option>
		</select>
		<input type="button" onclick="saveDashChart()" value="'.gettext("Save").'" class="btns" />
		<input type="button" onclick="hideDashChartForm()" value="'.gettext("Cancel").'" class="btns" />
	</div>
	</div>';
}
...
include_once $theme . '/charts.template.php';
?>