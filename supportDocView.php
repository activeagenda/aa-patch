...
$toc = '<b>'.gettext("Table of Contents").":</b><br/><ol>\n";
$content = '';
$counter = 1;
if('paged' == $mode){
    $tocLink = "<li><a href=\"supportDocView.php?{$qs}&amp;mode=paged&amp;sectionID=";
    $selectedSectionID = 0;
    ...
    $tocLink = "<li><a href=\"#section";
}
foreach($r as $row){
    $toc     .= $tocLink . $counter ."\">{$row['Title']}</a></li>\n";
...
if( !empty( $r['WikiGuide'] ) ){
	$externalLinks = '<b>'.gettext("External").":</b><br />\n";
	$externalLinks .= '<a href="'.$r['WikiGuide'].$r['WikiArticle'].'" target="_blank">'.gettext('Online Documentation')."</a>\n";
}
