...
function saveListSequence(&$result, $startRow)
{
    if( count($result) > 0 ){
		...
		$listSequence['countsql'] = $this->getCountSQL();
        $listSequence['rpp'] = $this->rowsPerPage;
		$listSequence['listsql'] = $this->getListSQL();
    ...
    )
{
    ...
    if(empty($rowLink)){
		...
		$this->showRowLinks = true;
        $this->rowLink = $rowLink;
    }
	
	if( empty($rowEditLink) ){
        $this->showEditRowLinks = false;
    }else{
		$this->showEditRowLinks = true;
        $this->rowEditLink = $rowEditLink;
    }
	...
	if( isset($this->formatOptions['akaID']) ){
        $this->akaID = $this->formatOptions['akaID'];
		$this->isModuleAlias = true;
    }else{
		$this->isModuleAlias = false;
	}
		
	$this->markerRecordID = $markerRecordID;
	$this->fixToolsWidth = $fixToolsWidth;
...
function render($startRow = null, $defaultOrderBys = null)
{
    ...
    if( $nRows > 0 ){
        ...
        if( $this->gridType == 'edit' || $this->gridType == 'edit_nfe' || $this->gridType == 'edit_noadd' || $this->gridType == 'edit_aqe' ){
            $content = sprintf(
                VIEWGRID_TABLE,
                $pageSelector . $headerHTML . $rowHTML
            );
        }elseif( $this->listData->rowsPerPage > 5 ){ //arbitrary number to decide when page selector needs to appear both at the top and bottom
            $content = sprintf(
                VIEWGRID_TABLE,
...
function renderHeaders($orderBys)
{
    ...
    foreach( $fieldNames as $fieldName ){
        ...
        if( 'hide' != $alignment ){
			...
			if( in_array( $this->moduleID, $globalModules ) ){
				$content .= sprintf(
					GRID_HEADER_CELL_GLOBAL,
    ...
    case 'edit_aqe':
	case 'edit':	
        $content = sprintf( EDITGRID_HEADER_CELL_ADDNEW, gettext("Add New"), $theme_web, $this->moduleID ).$content;
        break;
    case 'edit_nfe':
    case 'edit_noadd':
    case 'view':
...
function renderRows($startRow, $orderBys)
{
    ...
    case 'list':
    default:
		if( $this->showEditRowLinks ){
			$linkTemplate = LIST_GRID_NAVLINK_EDIT;
			$elmsg = gettext("Delete this record");
		}else{
			$linkTemplate = LIST_GRID_NAVLINK;			
		}	
		$msg = gettext("View this record");
		$rmmsg = gettext("Record menu");
    }
    ...
    foreach( $rows as $row_ix => $row ){
		...
		if( isset( $this->formatOptions['allowRecordIconsOn'] ) ){
			$editFilterField = $this->formatOptions['allowRecordIconsOn'];
			if( isset($row[$editFilterField]) ){
				if( $row[$editFilterField] == $_SESSION['User']->PersonID OR $_SESSION['User']->IsAdmin ){
					$this->showRowLinks = true;
				}else{
					$this->showRowLinks = false;
				}
        ...
        if( $this->showRowLinks ){
            if( $paging ){
				...
				$sequenceMenuID = '&sr='.($startRow + $row_ix);
            }else{
                $sequenceID = '';
				$sequenceMenuID = '';
            }

            if( 'edit_nfe' == $this->gridType OR 'edit_aqe' == $this->gridType ){
                $rowLink = '';
            }else{
                $rowLink = $this->rowLink . $sqQS .'&amp;rid='. $rowID . $sequenceID;
				$rowEditLink = $this->rowEditLink . $sqQS .'&amp;delete=1&amp;rid='. $rowID . $sequenceID;
				$rowMenuID = 'rid='. $rowID . $sequenceMenuID;
            }
            ...
            if( isset($this->headerPhrases[$fieldName]) ){
				...
				if('hide' != $alignment){
                    if( $this->useBestPractices && 'IsBestPractice' == $fieldName ){
						...
						$fieldValue = str_replace( '!mailtoRecordTopicSubject!', $mailtoRecordTopicSubject, $fieldValue );
						$fieldValue = str_replace( '!themeDirectory!', $theme_web, $fieldValue );
                        ...
                        if( !empty($this->linkFields[$fieldName]) ){
                            ...
                            if( !empty($linkValue) ){
								...
								if( $newWin ){
									...
									if( $this->moduleID == 'att' ){
										// very ugly hack ~{:-(
										$linkIcon = 'attachment.png';
									}else{
										$linkIcon = 'extlnk.png';
									}																	
									...
									if( $this->moduleID == 'cos' AND $fieldName == 'Tag'){
										// very ugly hack ~{:-(
										$postlinkIcon = ' <img src="'.$theme_web.'/img/follow.gif"/>';
									}else{
										$postlinkIcon = '';
									}
									$str_target = '';
                                ...
                                $fieldContent = "<a".$str_target." href=\"$link\">".$extlinkIcon.$fieldContent.$postlinkIcon."</a>";
							}
                        }
                    }
                    $rowContent .= sprintf(
...
function renderPageSelector($nRows, $currentRow = 0, $orderBys = null)
{
    ...
    $stats = sprintf(
        gettext("Rows: %s Pages: %s"),
		$nRows,
        $nPages        
        );
	
	$markerRecordNexPre = null;
	$markerRecordPager = null;
	if( isset( $this->markerRecordID ) ){
		$markerRecordNexPre = '&rid='.$this->markerRecordID;
		$markerRecordPager = '&amp;rid='.$this->markerRecordID;
	}
    if( $currentPage == 1 ){
        $prevContent = sprintf(
        ...
        $prevContent = sprintf(
            GRID_NAV_PREVLINKS,
            'javascript:updateList(\'list_'.$this->moduleID.'\',\''.$this->pagingLink.''.$srQS.$markerRecordNexPre.'&sr=0&rpc=1\');',
            'javascript:updateList(\'list_'.$this->moduleID.'\',\''.$this->pagingLink.''.$srQS.$markerRecordNexPre.'&sr='.($currentRow - $this->listData->rowsPerPage).'&rpc=1\');',
            $theme_web
            );
    }
    if( $currentPage == $nPages ){
        ...
        $postContent = sprintf(
            GRID_NAV_NEXTLINKS,
            'javascript:updateList(\'list_'.$this->moduleID.'\',\''.$this->pagingLink.$srQS.$markerRecordNexPre.'&sr='.($currentRow + $this->listData->rowsPerPage).'&rpc=1\');',
            'javascript:updateList(\'list_'.$this->moduleID.'\',\''.$this->pagingLink.$srQS.$markerRecordNexPre.'&sr='.($nRows - 1).'&rpc=1\');',
            $theme_web
            );
    }
    $innerContent = '<select class="edt" name="pager" onchange="updateList(\'list_'.$this->moduleID.'\',\''.$this->pagingLink.'\'+this.value+\'&amp;rpc=1'.$markerRecordPager.'\');">';
...
//        gettext('all') => -1,
    ...
    $pageRows .= ' <select class="edt" name="rpp" onchange="updateList(\'list_'.$this->moduleID.'\',\''.$this->pagingLink.'\'+this.value+\'&amp;rpc=1'.$markerRecordPager.'\');">';
