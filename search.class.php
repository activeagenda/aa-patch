...
function _removeConditions($userID, $table, $chartID = null)
{
    global $dbh;
    //$moduleID from $this->moduleID
    ...
    case 'dsbcc':
        if( empty($userID) ){
            $dbh->query('ROLLBACK');
            trigger_error("Search->_removeConditions: No userID specified.", E_USER_ERROR);
        }
        if( empty($chartID) ){
            $dbh->query('ROLLBACK');
            trigger_error("Search->_removeConditions: No chartID specified.", E_USER_ERROR);
        }
        $chartWhereSnip = " AND DashboardChartID = '$chartID'";        
        break;
    default:
        trigger_error("Search->_removeConditions: Table $table not supported.", E_USER_ERROR);
        break;
    }
    ...
    if( count($existingSet) > 0 ){
		$SQL = "UPDATE `$table` \n";
        $SQL .= "SET _Deleted = 1 \n";
        $SQL .= "WHERE UserID = $userID AND ModuleID = '{$this->moduleID}'$chartWhereSnip";
        $r = $dbh->query($SQL);
        dbErrorCheck($r);
    }
} //end _removeConditions

function _archiveSearchDefault( $userID )
{    
	global $dbh;
	
	$moduleID = $this->moduleID;
	$userID = intval( $userID );
	$searchPhrases = serialize( $this->phrases );
	$searchPostData = serialize( $this->postData );
	
	$SQL = "INSERT INTO `usrsh` (\n";
	$SQL .= "    ModuleID,\n";
	$SQL .= "    UserID,\n";
	$SQL .= "    SearchPhrases,\n";
	$SQL .= "    SearchPostData,\n";
...
function saveUserDefault($userID)
{
	...
	$this->_archiveSearchDefault( $userID );
    $this->isUserDefault = true;
    $_SESSION['Search_'.$this->moduleID] = $this;
}

function removeUserDefault($userID)
{
    $this->_removeConditions($userID, 'usrsd');
