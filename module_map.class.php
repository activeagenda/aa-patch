...
function &createObject($moduleID, $type = null, $callerRef = null)
{
    ...
    if( $valid === true ){
        if( empty($callerRef) ){
            $object = call_user_func_array( array($type, 'Factory'), array(& $this, $moduleID));
        }else{
...
function &createObjectWithRef($moduleID, $type = null, &$callerRef)
{
    ...
    if( $valid === true ){
        if( empty($callerRef) ){
            ...
            $object = call_user_func_array( array($type, 'Factory'), array(& $this, $moduleID, $callerRef) );
        }
...
//print "creating $type {$this->name}\n";
        $object = call_user_func_array( array($type, 'DocFactory'), array(& $this, $moduleID ));
        if( $object !== false ){
            return $object;
        }else{
